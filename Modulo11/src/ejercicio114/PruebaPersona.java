...
public class PruebaPersona {
	...
	public static void main(String[] args) {
		...
		Integer indice = ThreadLocalRandom.current().nextInt(1, 9);	
		System.out.println("Personas a generar: "+indice);
		
		for (int i = 0; i < indice; i++) {			
			Integer aleatorio = new Random().nextInt(1000000)+1;
			//si un número es múltiplo de 3 se genera con función que recibe String, si no con obtenerPersonaAleatoria()
			if (aleatorio%3 == 0){
	...
	static List<Persona> ordenarLista(List<Persona> personas){
		Queue<Persona> cola = new PriorityQueue<Persona>(personas);
